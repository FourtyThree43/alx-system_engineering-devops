#!/usr/bin/env bash
# Script: 4-mysql_config_replica.sh
# Description: Update MySQL Configuration on Replica Server
...
if grep -qE '^server-id\s*=\s*2' /etc/mysql/mysql.conf.d/mysqld.cnf && grep -qE '^relay-log\s*=\s*/var/log/mysql/mysql-relay-bin.log' /etc/mysql/mysql.conf.d/mysqld.cnf && grep -qE '^log_bin\s*=\s*/var/log/mysql/mysql-bin.log' /etc/mysql/mysql.conf.d/mysqld.cnf && grep -qE '^binlog_do_db\s*=\s*tyrell_corp' /etc/mysql/mysql.conf.d/mysqld.cnf; then
...
EOF
    ...
    if service mysql restart; then
        ...
        cp /etc/mysql/mysql.conf.d/mysqld.cnf ~/4-mysql_configuration_replica
    else
